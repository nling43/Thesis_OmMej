...
export default function NavBar() {
  const { onClear, onEdgesChange, nodes , onSelectNodes,selectedNodes} = useStore(selector, shallow);
    ...
    switch (number) {
      case 1:
        const questions = nodes.filter((node) => node.type.includes('question'));
 		const result_question = questions.filter((question) => question.data.text.sv.includes(search));	
      ...
      case 2:
		...
		const result_types = types.filter((type) => type.data.type.includes(search));
